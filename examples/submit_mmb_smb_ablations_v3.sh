#!/usr/bin/env bash
# submit_mmb_smb_ablations_v3.sh
# -----------------------------------------------------------------------------
# Convenience wrapper: submits all 8 variants in
# `DATASET_VARIANTS["mmb0-1b_smb1-1b_1p-ablations-v3"]` as separate LSF
# jobs (one bsub per variant, one GPU exclusive each).
#
# v3 spine: +wide+rvq-both+decoder-cov+adv+adj-w250
#   - `+wide`     was the niche-identification winner from v1
#   - `+adj-w250` was the batch-integration winner from v1
# Variants scan codebook size (RVQ k1=30/50/80, larger k2, 3-level) and
# revisit CVQ on the new spine.
#
...
#   DRY_RUN=1 bash examples/submit_mmb_smb_ablations_v3.sh
#
#   # Force a longer wallclock or more memory:
#   LSF_WALL=48:00 LSF_MEM_MB=200000 \
#       bash examples/submit_mmb_smb_ablations_v3.sh
#
...
#       mmb0-1b_smb1-1b_1p-ablations-v3/<variant>.{out,err}
...
exec bash "$SCRIPT_DIR/submit_dataset_sweep.sh" "mmb0-1b_smb1-1b_1p-ablations-v3" "$@"
