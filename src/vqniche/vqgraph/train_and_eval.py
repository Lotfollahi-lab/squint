...
def train_mini_batch_token(
    model,
    feats,
    codebook_embeddings,
    tea_soft_token_assignments_all,
    batch_size,
    criterion,
    optimizer,
    lamb=1,
...
def run_transductive(
    ...
    acc = evaluator(out[idx_test], labels[idx_test])
    logger.info(f"Best valid model at epoch: {best_epoch: 3d}, acc: {acc :.4f}")
...
def run_inductive(
    ...
    if "MLP" in model.model_name:
        ...
        out, _, acc_ind = evaluate_mini_batch(model, feats, labels, criterion, batch_size, evaluator, idx_test_ind)
    ...
    acc_tran = evaluator(obs_out[obs_idx_test], obs_labels[obs_idx_test])
    out[idx_obs] = obs_out
    logger.info(f"Best valid model at epoch: {best_epoch :3d}, acc_tran: {acc_tran :.4f}, acc_ind: {acc_ind :.4f}")
...
def distill_run_transductive(
    ...
    for epoch in range(1, conf["max_epoch"] + 1):
        ...
        loss_l = train_mini_batch(model, feats_l, labels_l, batch_size, criterion_l, optimizer, lamb_soft_labels)
        loss_t = train_mini_batch(model, feats_t, out_t, batch_size, criterion_t, optimizer, 1 - lamb_soft_labels)
        ...
        if epoch % conf["eval_interval"] == 0:
            ...
            _, loss_test, acc = evaluate_mini_batch(model, feats_test, labels_test, criterion_l, batch_size, evaluator)
            if epoch % 5 == 0:
                print(f"Ep {epoch:3d} | loss: {loss:.4f} | acc: {acc:.4f}")
                logger.info(f"Ep {epoch:3d} | loss: {loss:.4f} | acc: {acc:.4f}")
            loss_and_score += [[epoch, loss_l, loss_test, score_l, acc]]
            if acc >= best_acc:
                best_acc = acc
                state = copy.deepcopy(model.state_dict())
                count = 0
    ...
    model.load_state_dict(state)
    out, _, _ = evaluate_mini_batch(model, feats, labels, criterion_l, batch_size, evaluator, idx_val)
...
def distill_run_inductive(
    ...
    for epoch in range(1, conf["max_epoch"] + 1):
        ...
        if epoch % conf["eval_interval"] == 0:
            _, loss_l, score_l = evaluate_mini_batch(model, feats_l, labels_l, criterion_l, batch_size, evaluator)
            _, loss_val, acc = evaluate_mini_batch(model, feats_val, labels_val, criterion_l, batch_size, evaluator)
            _, loss_test_tran, acc_tran = evaluate_mini_batch(
                model,
                feats_test_tran,
                labels_test_tran,
            ...
            if epoch % 5 == 0:
                print(f"Ep {epoch:3d} | loss: {loss:.4f} | acc_tran: {acc_tran:.4f} | acc_ind: {acc_ind:.4f}")
                logger.info(f"Ep {epoch:3d} | loss: {loss:.4f} | acc_tran: {acc_tran:.4f} | acc_ind: {acc_ind:.4f}")
            loss_and_score += [
                [
                    epoch,
                    loss_l,
    ...
    model.load_state_dict(state)
    obs_out, _, acc = evaluate_mini_batch(model, obs_feats, obs_labels, criterion_l, batch_size, evaluator, obs_idx_val)
    out, _, acc_ind = evaluate_mini_batch(model, feats, labels, criterion_l, batch_size, evaluator, idx_test_ind)

    acc_tran = evaluator(obs_out[obs_idx_test], labels_test_tran)
    out[idx_obs] = obs_out

    logger.info(f"Best acc_tran: {acc_tran :.4f}, acc_ind: {acc_ind :.4f}")
